...
#     • ข้าม gacha4v2/gacha5v2 (= การสุ่มปกติ 1 ครั้งก่อนถึง gacha500) → ไม่เสียเหรียญกับรอบ 100
#     • กด gacha500 รอบเดียว แล้วออกจากลูปสุ่มเลย (ไม่วนกด gacha4 ต่อ)
...
# ── กันค้างหน้า title (play8) ───────────────────────
# ระหว่างรอ play8 / checkpointlogin ถ้า "ไม่เจอทั้งคู่" ติดกันนานเกินกี่วินาที
# = จอค้างอยู่หน้าที่บอทไม่รู้จัก (เกมค้าง / มีอะไรบังจอ / keymap overlay ของ MuMu โผล่)
#   ครบเวลา → กด Back + ปิดเกมเปิดใหม่ (ทำได้ 2 รอบ)
#   ยังไม่ขึ้นอีก รอบที่ 3 → ปล่อยไฟล์คืน input-id แล้วไปไฟล์ใหม่ (ไม่ต้องรอ TIMEOUT_MINUTES)
PLAY8_STUCK_SECS = 90
...
#
# *** 0 = ปิดการข้าม → เหรียญเท่าไหร่ก็สุ่มเสมอ (เหรียญ 0 ก็ยังเข้าไปสุ่ม) ***
#     เพราะเงื่อนไขคือ "เหรียญ < ค่านี้" ซึ่งไม่มีทางน้อยกว่า 0
GACHA_MIN_COIN = 0
...
# ── Auto Update แบบไม่ต้องมีคนกด ───────────────────
# True  = เจอเวอร์ชันใหม่ตอนเปิด login.bat → อัปเดตเองเงียบๆ ไม่มีหน้าต่างให้กด
#         เสร็จแล้ว login.bat เปิดบอทใหม่ให้เองอัตโนมัติ (เหมาะกับหลายสิบเครื่อง)
# False = มีหน้าต่างถามก่อนอัปเดต + หน้าต่างแจ้งเตือนตอนเสร็จ (ต้องเดินไปกดเองทีละเครื่อง)
AUTO_UPDATE_NO_ASK = True
